...
export async function getNegotiationStrategy(rfq: any, selectedQuote: any, allQuotes: any[]) {
  if (!ai) return "Negotiation strategy is unavailable.";

  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `
        You are ProcureMind AI, a master procurement negotiator.
        RFQ: ${JSON.stringify(rfq)}
        Target Vendor Quote: ${JSON.stringify(selectedQuote)}
        Competitor Quotes: ${JSON.stringify(allQuotes.filter(q => q.id !== selectedQuote.id))}
        
        Provide a "Negotiation Playbook" for this vendor.
        Include:
        1. Leverage Points (Why they should lower price or improve terms)
        2. Target Price Suggestion
        3. A professional draft email to the vendor.
    ...
    return "Failed to generate negotiation strategy.";
  }
}
