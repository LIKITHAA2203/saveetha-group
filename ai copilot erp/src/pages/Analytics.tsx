...
export default function Analytics() {
  ...
  const spendByMonth = pos.reduce((acc: any[], po) => {
    ...
    if (existing) {
      existing.amount += po.totalAmount || 0;
    } else {
      acc.push({ name: month, amount: po.totalAmount || 0 });
    }
    return acc;
  }, []).sort((a, b) => {
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fontWeight: 600, fill: '#14141440'}} tickFormatter={(value) => `₹${value}`} />
                <Tooltip 
