...
import { getRecommendationExplanation } from '../services/aiService';
...
export default function RFQs() {
  ...
  const [bulkVendors, setBulkVendors] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (showComparison && selectedRfq) {
      const generateExplanation = async () => {
        setIsGeneratingExplanation(true);
        const rfqQuotes = getQuotesForRfq(selectedRfq.id);
        const explanation = await getRecommendationExplanation(selectedRfq, rfqQuotes);
  ...
  const handleAward = async (quote: any, rfqToAward?: any) => {
    const targetRfq = rfqToAward || selectedRfq;
    try {
      // Create PO
      await addDoc(collection(db, 'purchaseOrders'), {
        ...
        itemName: targetRfq.itemName,
        quantity: targetRfq.quantity,
        unitPrice: quote.price,
        totalAmount: quote.price * targetRfq.quantity,
        status: 'pending',
  ...
  const handleQuickAward = async (rfq: any) => {
    const rfqQuotes = getQuotesForRfq(rfq.id);
    if (rfqQuotes.length === 0) return;

    // Find the best quote based on recommendation score
    const bestQuote = [...rfqQuotes].sort((a, b) => (b.recommendationScore || 0) - (a.recommendationScore || 0))[0];
    
    await handleAward(bestQuote, rfq);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="bg-white text-[#141414] px-6 py-3 rounded-xl font-bold border border-[#141414]/10 flex items-center gap-2 hover:bg-[#F5F5F5] transition-all"
          >
              ...
              <div className="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className="flex items-center gap-3">
                    {rfq.status !== 'awarded' && rfqQuotes.length > 0 && (
                      <button 
                        onClick={() => handleQuickAward(rfq)}
                        className="bg-green-600 text-white px-4 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-green-700 transition-all shadow-sm"
                        title="Award to top-scored vendor instantly"
                      >
                        <CheckCircle2 className="w-4 h-4" /> Quick Award
                      </button>
                    )}
                    {rfq.status !== 'awarded' && (
                      <button 
      ...
      {showBulkModal && (
        <div className="fixed inset-0 bg-[#141414]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[40px] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="p-10 border-b border-[#141414]/10 flex justify-between items-center">
              <div>
                <h2 className="text-3xl font-bold text-[#141414] flex items-center gap-3">
                  <FileSpreadsheet className="w-8 h-8 text-green-600" /> Confirm Bulk RFQs
                </h2>
                <p className="text-[#141414]/40 font-medium italic serif">Review {bulkData.length} items from CSV upload.</p>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8">
              <div className="bg-blue-50 border border-blue-100 p-6 rounded-3xl flex items-start gap-4">
                <AlertOctagon className="w-6 h-6 text-blue-600 mt-1" />
                <div>
                  <p className="text-sm font-bold text-blue-900 mb-1">CSV Format Required</p>
                  <p className="text-xs text-blue-800/60 leading-relaxed">
                    Ensure your CSV has headers: <code className="bg-white/50 px-1 rounded">itemName</code>, <code className="bg-white/50 px-1 rounded">quantity</code>, <code className="bg-white/50 px-1 rounded">requiredDate</code>, <code className="bg-white/50 px-1 rounded">priority</code>.
                  </p>
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-[#F5F5F5] text-[10px] uppercase tracking-widest font-bold text-[#141414]/40">
                      ...
                      <th className="px-8 py-6">Required Date</th>
                      <th className="px-8 py-6">Priority</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#141414]/5">
                    {bulkData.map((item, idx) => (
                      <tr key={idx} className="hover:bg-[#F5F5F5]/50 transition-all">
                        <td className="px-8 py-6 font-bold text-[#141414]">{item.itemName}</td>
                        <td className="px-8 py-6 font-bold text-[#141414]">{item.quantity}</td>
                        <td className="px-8 py-6 text-sm text-[#141414]/60">{item.requiredDate}</td>
                        <td className="px-8 py-6">
                          <span className={cn(
                            "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest",
                            item.priority === 'high' ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-600"
                          )}>
              ...
              <div className="bg-white p-8 rounded-3xl border border-[#141414]/10">
                <label className="block text-[10px] uppercase tracking-widest font-bold text-[#141414]/40 mb-4">Assign Vendors to All</label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {vendors.map(v => (
                    <label key={v.id} className="flex items-center gap-3 p-4 bg-[#F5F5F5] rounded-2xl cursor-pointer hover:bg-gray-100 transition-all">
                      <input 
                        type="checkbox" 
                        checked={bulkVendors.includes(v.id)}
                        onChange={(e) => {
                          if (e.target.checked) setBulkVendors([...bulkVendors, v.id]);
                          else setBulkVendors(bulkVendors.filter(id => id !== v.id));
                        }}
                        className="w-4 h-4 rounded border-gray-300 text-[#141414] focus:ring-[#141414]"
                      />
                      <span className="text-sm font-bold">{v.name}</span>
            ...
            <div className="p-10 bg-white border-t border-[#141414]/10 flex gap-4">
              <button 
                onClick={() => setShowBulkModal(false)}
                className="flex-1 px-6 py-4 border border-[#141414]/10 rounded-2xl font-bold hover:bg-[#F5F5F5] transition-all"
              >
                Cancel
              </button>
              <button 
                onClick={handleBulkSubmit}
                disabled={bulkVendors.length === 0}
                className="flex-1 px-6 py-4 bg-[#141414] text-white rounded-2xl font-bold hover:bg-[#141414]/90 transition-all disabled:opacity-50"
              >
                Generate {bulkData.length} RFQs
              </button>
            </div>
          </div>
        </div>
                            ...
                            <div className="w-12 h-12 rounded-full border-4 border-[#141414]/5 flex items-center justify-center relative">
                              <svg className="w-full h-full -rotate-90 absolute">
                                <circle 
                                  ...
                                  strokeDashoffset={125.6 * (1 - (quote.recommendationScore || 0) / 100)}
                                />
                              </svg>
                    ...
                    <div className="space-y-6">
                      {isGeneratingExplanation ? (
                        <div className="flex flex-col items-center justify-center py-12 space-y-4">
                          ...
                          <p className="text-xs text-white/40 font-bold uppercase tracking-widest">Generating Explanation...</p>
                        </div>
                      ) : (
                        <p className="text-white/80 leading-relaxed italic serif text-lg">
                          {aiExplanation || "The AI is analyzing the quotes to provide a recommendation..."}
                        </p>
                      )}
