...
export default function Dashboard() {
  ...
  const spendByMonth = pos.reduce((acc: any[], po) => {
    ...
    if (existing) {
      existing.spend += po.totalAmount || 0;
    } else {
      acc.push({ name: month, spend: po.totalAmount || 0, savings: (po.totalAmount || 0) * 0.15 }); // Simulated savings
    }
    return acc;
  }, []).sort((a, b) => {
  ...
  const chartData = spendByMonth.length > 0 ? spendByMonth : [
    { name: 'Jan', spend: 0, savings: 0 },
    { name: 'Feb', spend: 0, savings: 0 },
    { name: 'Mar', spend: 0, savings: 0 },
    { name: 'Apr', spend: 0, savings: 0 },
    { name: 'May', spend: 0, savings: 0 },
    { name: 'Jun', spend: 0, savings: 0 },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Spend', value: `₹${stats.totalSpend.toLocaleString()}`, icon: IndianRupee, color: 'bg-blue-50 text-blue-600', trend: '+12.5%', up: true },
